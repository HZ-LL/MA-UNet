...
from numpy import *
from MAUNet import MA
# from network.UNet import UNet
from torch.utils.data import DataLoader
from dataset import GetDataset
from utils import check_accuracy, save_preds_imgs, DiceLoss
...
if __name__ == "__main__":
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(all_dataset)):
        print(f'****************************Fold:{fold}********************************')
        train_data = torch.utils.data.SequentialSampler(train_ids)
        val_data = torch.utils.data.SequentialSampler(val_ids)

        train_loader = DataLoader(
            all_dataset,
            batch_size = batch_size,
            num_workers = num_workers,
            pin_memory = True,  # 数据是否加载到CUDA固定内存页中
            sampler = train_data,
            drop_last = True
        )
        ...
        specificity_list = []
        auc_list = []
        # 如果loading设置为True，只有成功加载才能训练，否则抛出异常
        if loading:
            # 预训练权重不存在抛出异常
            if not os.path.exists(weights_path):
                raise FileNotFoundError(f"Not found pre-trained weights:{weights_path}")
            # 预训练权重加载失败抛出异常
            try:
